...
export class NorthwestCorner {
    constructor(supply, demand, costs) {
        this.supply = supply;
        this.demand = demand;
        this.costs = costs;
        this.solution = this.initializeSolution();
    }

    initializeSolution() {
        const solution = [];
        for (let i = 0; i < this.supply.length; i++) {
            solution.push(new Array(this.demand.length).fill(0));
        }
        return solution;
    }
    ...
    solve() {
        ...
        while (i < this.supply.length && j < this.demand.length) {
            const amount = Math.min(this.supply[i], this.demand[j]);
            this.solution[i][j] = amount;

            this.supply[i] -= amount;
            this.demand[j] -= amount;
    ...
    printSolution() {
        ...
        this.printMatrix(this.costs);

        console.log('\nAsignaciones:');
        for (let i = 0; i < this.supply.length; i++) {
            for (let j = 0; j < this.demand.length; j++) {
                if (this.solution[i][j] > 0) {
                    console.log(`Desde el Origen ${i + 1} al Destino ${j + 1}: ${this.solution[i][j]} unidades`);
                }
            }
        }
    }
...
// Ejemplo de uso:
const supply = [20, 30, 50];
const demand = [30, 40, 30, 20];
const costs = [
    [5, 7, 8, 10],
    [12, 10, 6, 8],
    [6, 8, 9, 7]
...
const northwestCorner = new NorthwestCorner(supply, demand, costs);
const result = northwestCorner.solve();
northwestCorner.printSolution();
